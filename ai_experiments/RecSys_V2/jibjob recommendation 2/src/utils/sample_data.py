...
def generate_users(n_users: int = 100, professional_ratio: float = 0.7) -> pd.DataFrame:
    """
    Generate synthetic user data with user types (client vs professional).
    
    Args:
        n_users: Number of users to generate
        professional_ratio: Ratio of professionals to total users
    ...
    for i in range(1, n_users+1):
        # Determine user type based on professional_ratio
        user_type = "professional" if i <= int(n_users * professional_ratio) else "client"
        ...
        if user_type == "professional":
            ...
            user['categories'] = np.random.choice(job_categories, num_categories, replace=False).tolist()
            user['experience_years'] = np.random.randint(1, 20)
            user['rating'] = round(np.random.uniform(3.0, 5.0), 1)
        else:
            # Clients don't have these fields
