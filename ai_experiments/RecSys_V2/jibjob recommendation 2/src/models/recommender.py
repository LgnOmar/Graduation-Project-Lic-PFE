...
class JobRecommender:
    ...
    def recommend(
        ...
        # rated_job_ids for specific user (actual IDs, not indices)
        ...
        """Generate recommendations using HeterogeneousGCN."""
        if self.graph is None or not isinstance(self.graph_model, HeterogeneousGCN):
            raise ValueError("HeterogeneousGCN model not trained or graph not built. Call train() first.")
            
        if user_id not in self.user_to_idx:
            logger.warning(f"User ID {user_id} not in mappings. Cannot generate recommendations.")
            return []
            
        user_idx = self.user_to_idx[user_id] # Single integer
        
        self.graph_model.eval()
        with torch.no_grad():
            # All job indices that exist in our current graph_model's understanding
            num_mapped_jobs = len(self.idx_to_job) # Or self.all_job_bert_embeddings.shape[0]
            all_job_indices_tensor = torch.arange(num_mapped_jobs, device=self.device)
            
            # Repeat user_idx for all jobs to form pairs
            user_indices_tensor = torch.full_like(all_job_indices_tensor, fill_value=user_idx)
            
            # Get predictions from HeteroGCN
            # Pass the full graph, and the specific user/job indices to predict for
            predicted_scores = self.graph_model(self.graph.to(self.device), 
                                                user_indices_tensor, 
                                                all_job_indices_tensor)
            
            # Exclude rated jobs
            candidate_job_indices = all_job_indices_tensor
            if exclude_rated and rated_job_ids_for_user:
                rated_job_indices_for_user_tensor = torch.tensor(
                    [self.job_to_idx[j_id] for j_id in rated_job_ids_for_user if j_id in self.job_to_idx],
                    dtype=torch.long
                ).to(self.device)
                
                # Create a mask for non-rated jobs
                # Convert to set for faster lookup if many rated items
                # For PyTorch tensor masking:
                is_not_rated_mask = torch.ones(num_mapped_jobs, dtype=torch.bool, device=self.device)
                if rated_job_indices_for_user_tensor.numel() > 0:
                    is_not_rated_mask[rated_job_indices_for_user_tensor] = False
                
                predicted_scores = predicted_scores[is_not_rated_mask]
                candidate_job_indices = all_job_indices_tensor[is_not_rated_mask]

            if predicted_scores.numel() == 0:
                return [] # No candidate jobs left

            # Get top-k
            if len(predicted_scores) <= top_k:
                top_scores_tensor, sorted_indices_within_candidates = torch.sort(predicted_scores, descending=True)
            else:
                top_scores_tensor, sorted_indices_within_candidates = torch.topk(predicted_scores, k=top_k)

            # Map candidate indices back to original job indices
            top_job_original_indices = candidate_job_indices[sorted_indices_within_candidates].cpu().tolist()
            final_scores = top_scores_tensor.cpu().tolist()

        recommendations = []
        for job_idx, score in zip(top_job_original_indices, final_scores):
            job_id_actual = self.idx_to_job.get(job_idx)
            if job_id_actual: # Check if job_idx is valid
                 recommendations.append({'job_id': job_id_actual, 'score': float(score)})
        
        return recommendations
        # The evaluate method will need careful adjustment to work with HeteroData
        # and how HeteroGCN makes predictions for batches of users for evaluation.
        # The current `graph_model.recommend` used by evaluate might be too slow if it does one user at a time.
        # The MAE/RMSE part of evaluate also uses single user-item GCN calls repeatedly.
    ...
    def evaluate(
        ...
        if rating_col in test_interactions.columns and not test_interactions.empty:
            ...
            if num_to_sample > 0:
                ...
                logger.info(f"EVAL DEBUG: Sample (normalized) ratings from test_interactions['{rating_col}']: {sample_ratings}")
                logger.info(f"EVAL DEBUG: Min/Max (normalized) ratings in test_interactions['{rating_col}']: [{test_interactions[rating_col].min():.4f}, {test_interactions[rating_col].max():.4f}]")
            else:
                logger.info(f"EVAL_DEBUG_RATINGS: No non-NA ratings found in test_interactions['{rating_col}'] to sample.")
                ...
                if relevant_job_ids_for_user:
                    ...
                    for true_relevant_job_id in relevant_job_ids_for_user:
                        if true_relevant_job_id in self.job_to_idx:
                            ...
                            user_idx_tensor = torch.tensor([current_user_idx], device=self.device) # Use current_user_idx
    ...
    # This goes INSIDE the JobRecommender class in src/models/recommender.py
    # Replacing the OLD find_similar_jobs method body


    def find_similar_jobs(self,
                        job_id: Any,
                        # We add jobs_df_for_similarity to pass the job texts
                        jobs_df_for_similarity: Optional[pd.DataFrame] = None,
                        top_k: int = 5
                        ) -> list:
        """
        Find the most similar jobs to a given job using cosine similarity of BERT embeddings.
        Args:
            job_id: The job ID to find similarities for.
            jobs_df_for_similarity: DataFrame containing all jobs with 'job_id', 'title', 'description'.
                                    This DataFrame should contain the enriched text.
            top_k: Number of similar jobs to return.
        Returns:
            List of tuples (job_id, similarity) sorted by similarity descending.
        """
        if jobs_df_for_similarity is None:
            logger.error("ERROR in find_similar_jobs: jobs_df_for_similarity was not provided. Cannot fetch job texts.")
            raise ValueError("jobs_df_for_similarity must be provided for this BERT-based similarity test.")

        # Get target job's text
        try:
            target_job_row = jobs_df_for_similarity[jobs_df_for_similarity['job_id'] == job_id]
            if target_job_row.empty:
                raise ValueError(f"Target Job ID {job_id} not found in the provided jobs_df_for_similarity.")
            target_job_info = target_job_row.iloc[0]
        except Exception as e:
            logger.error(f"Error fetching target job {job_id}: {e}")
            return [] # Return empty list on error

        # Combine title and description for BERT. Use 'cleaned_' versions if your process_job_descriptions adds them
        # and they are what BERT should ideally process. For now, assume direct use.
        target_title = str(target_job_info.get('title', ''))
        target_description = str(target_job_info.get('description', ''))
        target_text_for_bert = (target_title + " " + target_description).strip()

        if not target_text_for_bert:
            logger.warning(f"Target Job ID {job_id} has empty title and description. Cannot compute meaningful similarity.")
            return []

        logger.info(f"Computing BERT embedding for target job: '{job_id}' ('{target_text_for_bert[:70]}...')")
        target_emb = self.embedding_model.get_embeddings(target_text_for_bert) # BERTEmbeddings from self

        similarities = []
        
        # Prepare texts of other jobs
        other_job_ids_for_sim = []
        other_job_texts_for_bert = []

        for _, row in jobs_df_for_similarity.iterrows():
            other_id = row['job_id']
            if other_id == job_id: # Don't compare job to itself
                continue
            
            other_title = str(row.get('title', ''))
            other_description = str(row.get('description', ''))
            other_text = (other_title + " " + other_description).strip()

            if other_text: # Only consider jobs with some text
                other_job_ids_for_sim.append(other_id)
                other_job_texts_for_bert.append(other_text)

        if not other_job_texts_for_bert:
            logger.info(f"No other jobs found with text content to compare with {job_id}.")
            return []

        # Get BERT embeddings for all other jobs in a batch
        logger.info(f"Computing BERT embeddings for {len(other_job_texts_for_bert)} other jobs (for similarity)...")
        other_embs_matrix = self.embedding_model.batch_get_embeddings(other_job_texts_for_bert, batch_size=32) # Use batch_get_embeddings

        # Calculate cosine similarity
        for i, other_id in enumerate(other_job_ids_for_sim):
            # other_embs_matrix[i] is a 1D array (embedding). Reshape for dot product if target_emb is [1, dim].
            # target_emb is already [1, dim] from get_embeddings.
            # other_embs_matrix[i:i+1] is [1, dim]
            current_other_emb = other_embs_matrix[i:i+1] 
            
            sim_dot = np.dot(target_emb, current_other_emb.T) # Result is [[value]]
            norm_target = np.linalg.norm(target_emb)
            norm_other = np.linalg.norm(current_other_emb)
            
            if norm_target == 0 or norm_other == 0:
                similarity_score = 0.0
            else:
                similarity_score = sim_dot[0,0] / (norm_target * norm_other)
            
            similarities.append((other_id, float(similarity_score)))
            
        # Sort by similarity (highest first)
        similarities.sort(key=lambda x: x[1], reverse=True)
        
        if similarities:
            logger.info(f"Top BERT-similar job to '{job_id}': {similarities[0][0]} with score {similarities[0][1]:.4f}")
